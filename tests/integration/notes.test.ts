...
describe('integration: notes via real SDK (offline mode)', () => {
  ...
  beforeEach(async () => {
    ...
    // Offline mode: hand-init the SDK directly so we can point at the
    // pre-existing fixture budget without going through client.init().
    await (api.init as unknown as (cfg: unknown) => Promise<unknown>)({ dataDir: tmp });
    ...
    // Construct the client with initialized=true so shutdown() works correctly.
